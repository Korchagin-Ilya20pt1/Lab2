...
inline std::wstring modAlphaCipher::getValidKey(const std::wstring & s)// реализация метода для проверки ключа 
                                                                       //на валидность и перевода в нем символы 
                                                                       //в верхний регистр, если это необходимо
{
    if (s.empty())
...
inline std::wstring modAlphaCipher::getValidOpenText(const std::wstring & s)//реализация метода для валидации открытого текста
{
    std::wstring tmp;
    for (auto c:s) {
        if (isalpha(c)) {
